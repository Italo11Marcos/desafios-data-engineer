...
class S3Client:
    def __init__(self):
        ...
        for var in self._envs:
            if self._envs[var] is None:
                logger.warning(f"A variável de ambiente {var} não está definida.")
                sys.exit(1)
    ...
    def download_file(self, s3_key):
        try:
            ...
            logger.success(f"Download bem-sucedido para {s3_key}")
            return file
        except NoCredentialsError:
            logger.error(
                "Credenciais não encontradas. Certifique-se de configurar suas credenciais AWS corretamente."
            ...
            logger.error(f"Ocorreu um erro durante o download: {e}")
